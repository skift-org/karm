...
Res<> Name::write(Io::Stream& w) const {
    return Io::format(w, "/{}", str());
}

Res<> Array::write(Io::Stream& w) const {
    try$(Io::format(w, "["));
...
Res<> Dict::write(Io::Stream& w) const {
    try$(Io::format(w, "<<\n"));
...
Res<> Value::write(Io::Stream& w) const {
...
Res<> File::write(Io::Stream& writer) const {
...
Res<> XRef::write(Io::Stream& w) const {
    try$(Io::format(w, "1 {}\n", entries.len()));
