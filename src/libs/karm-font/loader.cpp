...
export Res<Rc<Gfx::Fontface>> loadFontface(Mime::Url url) {
    auto file = try$(Sys::File::open(url));
    auto map = try$(Sys::mmap().map(file));
    return loadFontface(std::move(map));
}

export Res<Rc<Gfx::Fontface>> loadFontfaceOrFallback(Mime::Url url) {
...
export Res<Gfx::Font> loadFont(f64 size, Mime::Url url) {
    return Ok(Gfx::Font{
...
export Res<Gfx::Font> loadFontOrFallback(f64 size, Mime::Url url) {
    return Ok(Gfx::Font{
