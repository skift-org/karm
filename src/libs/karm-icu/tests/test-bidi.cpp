...
test$("bidiTestFileLevels") {
    ...
    for (auto testCase : testCasesFromFile(file)) {
        ...
        auto [paragraph, _] = bidi._prepareParagraph(inputParagraph);
        auto output = bidi._computeLevelsFromParagraph(inputParagraph, paragraph, testCase.paragraphLevel);

        expectEq$(output.len(), testCase.levels.len());
        bool isOk = true;
        for (usize i = 0; i < output.len(); i++) {
            if (not testCase.levels[i])
                continue;

            if (output[i] != *testCase.levels[i]) {
                isOk = false;
                logDebug("Test case {} failed at index {}: expected {}, got {}", testCount, i, *testCase.levels[i], output[i]);
                logDebug("Test case: {}, Output: {}", testCase, output);
                expectEq$(output[i], *testCase.levels[i]);
                break;
            }
        }
    ...
    logDebug("Bidi test cases from file ({}/{})", correct, testCount);
...
test$("bidiTestFileReorder") {
    ...
    for (auto testCase : testCasesFromFile(file)) {
        ...
        bidi.reorderLine(mutSub(levels), [&](auto level) {
            return level.v1;
        });
