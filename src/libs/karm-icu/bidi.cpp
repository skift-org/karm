...
export struct Bidi {
    ...
    Vec<usize> explicitLevelsAndDirections(MutSlice<BidiType> paragraph, usize paragraphLevel, Vec<Opt<usize>> const& matchingPDIIndices) {
        ...
        directionalStatusStack.pushBack({.embeddingLevel = paragraphLevel, .directionOverrideStatus = DirectionOverrideStatus::NEUTRAL, .directionIsolateStatus = false});
    ...
    Vec<IsolatingRunSequence> computeIsolatingRunSequence(MutSlice<BidiType> paragraph, Vec<usize>& levels, Vec<Opt<usize>> const& matchingPDIIndices) {
    ...
    Vec<Pair<usize>> findBracketPairs(Slice<Rune> input, MutSlice<BidiType> paragraph, IsolatingRunSequence const& isolatingRunSequence) {
    ...
    void _changeAdjacentTo(MutSlice<BidiType> paragraph, IsolatingRunSequence const& isolatingRunSequence, auto predToChange, auto predAdjacent, BidiType newType) {
        usize prevValidIndex = paragraph.len();

        auto transformAdjacent = [&](usize i) {
            if (prevValidIndex == paragraph.len()) {
                prevValidIndex = i;
                return;
            }

            if (predAdjacent(paragraph, prevValidIndex) and predToChange(paragraph, i)) {
                paragraph[i] = newType;
            }
    ...
    void resolveWeakTypes(MutSlice<BidiType> paragraph, IsolatingRunSequence const& isolatingRunSequence) {
        ...
        usize lastIndexOfInterest = paragraph.len();
        for (auto const& levelRun : isolatingRunSequence.levelRuns) {
            for (usize i = levelRun.start; i < levelRun.end(); i++) {
                if (paragraph[i] == BidiType::EN) {
                    if (lastIndexOfInterest != paragraph.len() and paragraph[lastIndexOfInterest] == BidiType::AL)
        ...
        prevValidIndex = paragraph.len();
        usize prevPrevValidIndex = paragraph.len();
        for (auto const& levelRun : isolatingRunSequence.levelRuns) {
            for (usize i = levelRun.start; i < levelRun.end(); i++) {
                if (removeDueToX9(paragraph[i]))
                    continue;

                if (prevPrevValidIndex == paragraph.len()) {
        ...
        _changeAdjacentTo(paragraph, isolatingRunSequence, [](MutSlice<BidiType> paragraph, usize i) {
        ...
        lastIndexOfInterest = paragraph.len();
        for (auto const& levelRun : isolatingRunSequence.levelRuns) {
            for (usize i = levelRun.start; i < levelRun.end(); i++) {
                if (removeDueToX9(paragraph[i]))
                    continue;
                ...
                if (paragraph[i] == BidiType::EN) {
                    if (lastIndexOfInterest == paragraph.len()) {
    ...
    BidiType _applyNeutralTypesScopeChanges(BidiType type) {
    ...
    void executeN0ForBracketPair(Slice<Rune> input, MutSlice<BidiType> paragraph, IsolatingRunSequence const& isolatingRunSequence, Pair<usize> const& bracketPair, usize startLevelRun, BidiType lastStrongBeforeBracket) {
        ...
        auto changeFollowingNSM = [&](usize i, BidiType newType) {
            while (
                ...
                ((getBidiType(input[i + 1]) == BidiType::NSM and paragraph[i + 1] == BidiType::ON) or getBidiType(input[i + 1]) == BidiType::BN
                )
            ) {
        ...
        for (usize j = startLevelRun; j < isolatingRunSequence.levelRuns.len(); j++) {
            ...
            for (usize i = j == startLevelRun ? bracketPair.v0 : levelRun.start; i < levelRun.end(); i++) {
                ...
                auto pi = _applyNeutralTypesScopeChanges(paragraph[i]);
    ...
    void resolveNeutralTypes(Slice<Rune> input, MutSlice<BidiType> paragraph, IsolatingRunSequence const& isolatingRunSequence, Vec<usize>&) {
            ...
            for (usize j = 0; j < isolatingRunSequence.levelRuns.len(); j++) {
                ...
                for (usize i = levelRun.start; i < levelRun.end(); i++) {
                    ...
                    if (bracketPairs[bracketPairIndex].v0 == i) {
                        executeN0ForBracketPair(input, paragraph, isolatingRunSequence, bracketPairs[bracketPairIndex], j, lastStrongBeforeCurrentBracket);
        ...
        auto finishNIseq = [&](BidiType before, BidiType after, Pair<usize> startSeq, Pair<usize> endSeq) {
            if (before != after)
                return;
        ...
        for (usize j = 0; j < isolatingRunSequence.levelRuns.len(); j++) {
            ...
            for (usize i = levelRun.start; i < levelRun.end(); i++) {
                ...
                if (isNI(paragraph[i])) {
                    ...
                    if (insideNIseq)
                        finishNIseq(strongBeforeNIseq, _applyNeutralTypesScopeChanges(paragraph[i]), startOfNIseq, {i, j});
                    insideNIseq = false;
                    strongBeforeNIseq = _applyNeutralTypesScopeChanges(paragraph[i]);
                }
            }
        }
        if (insideNIseq) {
            finishNIseq(strongBeforeNIseq, isolatingRunSequence.eos, startOfNIseq, {last(isolatingRunSequence.levelRuns).end(), isolatingRunSequence.levelRuns.len() - 1});
        }
    ...
    Vec<usize> _computeLevelsFromParagraph(Slice<Rune> inputParagraph, MutSlice<BidiType> paragraph, usize paragraphLevel) {
        ...
        for (auto const& isolatingRunSequence : isolatingRunSequences) {
            ...
            resolveNeutralTypes(inputParagraph, paragraph, isolatingRunSequence, levels);
    ...
    Tuple<Vec<BidiType>, usize> _prepareParagraph(Slice<Rune> inputParagraph) {
    ...
    Tuple<Vec<usize>, paragraphLevel> computeLevels(Slice<Rune> inputParagraph) {
        auto [paragraph, paragraphLevel] = _prepareParagraph(inputParagraph);
        return _computeLevelsFromParagraph(inputParagraph, paragraph, paragraphLevel);
    }

    // https://unicode.org/reports/tr9/#L1
    template <typename T>
    void resetSomeLevelsForLine(MutSlice<T> levels, auto getLevel, Slice<Rune> inputLine, usize paragraphLevel) {
        auto [line, _] = _prepareParagraph(inputLine);

        //  On each line, reset the embedding level of the following characters to the paragraph embedding level:
        usize startOfIsolate = line.len();
        for (usize i = line.len() - 1; i != usize(-1); i--) {
            if (isIsolateInitiator(line[i]) or line[i] == BidiType::PDI) {
                startOfIsolate = i;
            } else if (line[i] == BidiType::S or line[i] == BidiType::B) {
                if (startOfIsolate != line.len()) {
                    for (usize j = i; j <= startOfIsolate; j++) {
                        getLevel(levels[j]) = paragraphLevel;
                    }
                }
                startOfIsolate = line.len();
            } else {
                startOfIsolate = line.len();
            }
        }
        if (startOfIsolate != line.len()) {
            for (usize j = 0; j <= startOfIsolate; j++) {
                getLevel(levels[j]) = paragraphLevel;
            }
        }
    }
    ...
    template <typename T>
    void reorderLine(MutSlice<T> line, auto getLevel) {
        usize lowestOddLevel = Limits<usize>::MAX;
        for (auto el : line) {
            if (auto level = getLevel(el); level % 2 == 1) {
                lowestOddLevel = min(lowestOddLevel, level);
            }
        }

        usize maxVisted = INT_MAX;
        while (true) {
            usize highestLevel = 0;
            for (auto el : line) {
                if (getLevel(el) >= maxVisted) {
                    continue;
                }
                highestLevel = max(highestLevel, getLevel(el));
            }
            ...
            maxVisted = highestLevel;

            Vec<urange> intervalsToReverse;
            for (usize i = 0; i < line.len(); i++) {
                if (getLevel(line[i]) >= highestLevel) {
                    usize start = i;
                    while (i < line.len() and getLevel(line[i]) >= highestLevel) {
                        i++;
                    }
                    intervalsToReverse.pushBack(urange::fromStartEnd(start, i));
                    i--;
                }
            }

            for (auto const& interval : intervalsToReverse) {
                reverse(mutSub(line, interval));
            }
        }
    }
};
