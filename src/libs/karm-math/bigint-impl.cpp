...
void _add(IBig& lhs, usize rhs) {
    ...
    if (lhs._sign == Sign::POSITIVE) {
        _add(lhs._value, rhs);
        return;
    }

    // lhs is negative: (-|lhs|) + rhs == rhs - |lhs|
    UBig urhs{rhs};
    if (lhs._value > urhs) {
        if (_sub(lhs._value, rhs) == SubResult::UNDERFLOW) [[unlikely]]
            panic("unexpected underflow");
        lhs._sign = Sign::NEGATIVE;
    } else if (lhs._value < urhs) {
        UBig tmp{rhs};
        if (_sub(tmp, lhs._value) == SubResult::UNDERFLOW) [[unlikely]]
            panic("unexpected underflow");
        lhs._value = std::move(tmp);
        lhs._sign = Sign::POSITIVE;
    } else {
        lhs._value = 0_ubig;
        lhs._sign = Sign::POSITIVE;
