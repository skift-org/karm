export module Karm.Core:math.funcs;
...
export constexpr auto lerp(auto a, auto b, auto p) {
    return a + (b - a) * p;
}
...
export constexpr bool isNan(Meta::Float auto x) {
    return __builtin_isnan(x);
}

export constexpr bool isInf(Meta::Float auto x) {
    return __builtin_isinf(x);
}

export constexpr bool isNegInf(Meta::Float auto x) {
...
export template <Meta::Integral T = isize, Precision = PRECISE>
...
export template <Meta::Float T = f64, Precision = PRECISE>
constexpr T pow(T base, T exponent) {
    if (isNan(exponent))
        return exponent;
    if (exponent == 0)
