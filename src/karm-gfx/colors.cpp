...
export constexpr Color ALPHA = {0, 0, 0, 0};
export constexpr Color BLACK = {0, 0, 0, 255};
export constexpr Color WHITE = {255, 255, 255, 255};

export using ColorRamp = Array<Color, 11>;

#define MAKE_COLOR_RAMP(NAME, COLORS...)                                  \
    export constexpr ColorRamp NAME##_RAMP = [](auto... v) -> ColorRamp { \
        return {Color::fromHex(v)...};                                    \
    }(COLORS);                                                            \
    export constexpr Color NAME = NAME##_RAMP[5];                         \
    export constexpr Color NAME##50 = NAME##_RAMP[0];                     \
    export constexpr Color NAME##100 = NAME##_RAMP[1];                    \
    export constexpr Color NAME##200 = NAME##_RAMP[2];                    \
    export constexpr Color NAME##300 = NAME##_RAMP[3];                    \
    export constexpr Color NAME##400 = NAME##_RAMP[4];                    \
    export constexpr Color NAME##500 = NAME##_RAMP[5];                    \
    export constexpr Color NAME##600 = NAME##_RAMP[6];                    \
    export constexpr Color NAME##700 = NAME##_RAMP[7];                    \
    export constexpr Color NAME##800 = NAME##_RAMP[8];                    \
    export constexpr Color NAME##900 = NAME##_RAMP[9];                    \
    export constexpr Color NAME##950 = NAME##_RAMP[10];
...
export constexpr Array<ColorRamp, 22> RAMPS = {
...
export constexpr Array<Color, 22> COLORS = {
...
export Color randomColor(Math::Rand& rand) {
...
export Color rainbowColor() {
    static f64 hue = 0;
    auto res = hsvToRgb({hue, 1, 1});
    hue += 1;
    if (hue > 360)
        hue = 0;
    return res;
