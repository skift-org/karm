...
Res<Rc<Gfx::Image>> loadBmp(Bytes bytes) {
    auto bmp = try$(Bmp::Decoder::init(bytes));
    auto img = Gfx::Image::alloc({bmp.width(), bmp.height()});
    try$(bmp.decode(*img));
    return Ok(img);
}

Res<Rc<Gfx::Image>> loadQoi(Bytes bytes) {
    auto qoi = try$(Qoi::Decoder::init(bytes));
    auto img = Gfx::Image::alloc({qoi.width(), qoi.height()});
    try$(qoi.decode(*img));
    return Ok(img);
}

Res<Rc<Gfx::Image>> loadPng(Bytes bytes) {
    auto png = try$(Png::Decoder::init(bytes));
    auto img = Gfx::Image::alloc({png.width(), png.height()});
    try$(png.decode(*img));
    return Ok(img);
}

Res<Rc<Gfx::Image>> loadJpeg(Bytes bytes) {
    auto jpeg = try$(Jpeg::Decoder::init(bytes));
    auto img = Gfx::Image::alloc({jpeg.width(), jpeg.height()});
    try$(jpeg.decode(*img));
    return Ok(img);
}

Res<Rc<Gfx::Image>> loadTga(Bytes bytes) {
    auto tga = try$(Tga::Decoder::init(bytes));
    auto img = Gfx::Image::alloc({tga.width(), tga.height()});
    try$(tga.decode(*img));
    return Ok(img);
}

Res<Rc<Gfx::Image>> loadGif(Bytes bytes) {
    auto gif = try$(Gif::Decoder::init(bytes));
    auto img = Gfx::Image::alloc({gif.width(), gif.height()});
    try$(gif.decode(*img));
    return Ok(img);
}
...
export Res<Rc<Gfx::Image>> load(Bytes bytes) {
...
export Res<Rc<Gfx::Image>> load(Ref::Url url) {
    if (url.scheme == "data") {
...
export Res<Rc<Gfx::Image>> loadOrFallback(Ref::Url url) {
    if (auto result = load(url); result)
        return result;
    return Ok(Gfx::Image::fallback());
}
