...
struct Atomic {
    ...
    always_inline T xchg(T desired, MemOrder order = SEQ_CST) {
        return __atomic_exchange_n(&_val, desired, order);
    }

    always_inline bool cmpxchg(T expected, T desired, MemOrder order = SEQ_CST) {
    ...
    always_inline T fetchAdd(T desired, MemOrder order = SEQ_CST) {
        return __atomic_fetch_add(&_val, desired, order);
    }

    always_inline T fetchSub(T desired, MemOrder order = SEQ_CST) {
        return __atomic_fetch_sub(&_val, desired, order);
    }

    always_inline T fetchInc(MemOrder order = SEQ_CST) {
        return __atomic_fetch_add(&_val, 1, order);
    }

    always_inline void inc(MemOrder order = SEQ_CST) {
        __atomic_add_fetch(&_val, 1, order);
    }

    always_inline void dec(MemOrder order = SEQ_CST) {
        __atomic_sub_fetch(&_val, 1, order);
    }

    always_inline T fetchDec(MemOrder order = SEQ_CST) {
        return __atomic_fetch_sub(&_val, 1, order);
    }

    always_inline T load(MemOrder order = SEQ_CST) const {
        return __atomic_load_n(&_val, order);
    }

    always_inline void store(T desired, MemOrder order = SEQ_CST) {
